...
public class Enemy_2 : Enemy //Enemy_2 also extends the Enemy class
{
    ...
    void Start()
    {
        ...
        //Set the birthTime to the crrent time
        birthTime = Time.time;
    ...
    {
        //Linear interppolations work based on a u value between 0&1
        float u = (Time.time - birthTime) / lifeTime;
        //If u > 1, then it has been longer than lifeTime since birthTime
        if (u > 1)
        {
            //Enemy_2 has finished its life
        ...
        //Adjust u by adding a U cCurve based on a Sine wave
        u = u + sinEccentricity * (Mathf.Sin(u * Mathf.PI * 2));

        //Interpolate the tow linear interpolation points
        pos = (1 - u) * p0 + u * p1;
        //Note that Enemy_2 does NOT call the base.Move() methd
    }
